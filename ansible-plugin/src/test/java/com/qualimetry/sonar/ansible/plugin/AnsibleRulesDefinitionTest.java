...
class AnsibleRulesDefinitionTest {
    ...
    @Test
    void shouldHaveMarkdownDocForEveryRule() {
        Path rulesDir = locateDocsRulesDir();
        assertThat(CheckList.getAllRuleKeys()).isNotEmpty();
        for (String ruleKey : CheckList.getAllRuleKeys()) {
            Path doc = rulesDir.resolve(ruleKey + ".md");
            assertThat(Files.exists(doc))
                    .as("Missing docs/rules/%s.md (run: node scripts/generate-rule-docs.js)", ruleKey)
                    .isTrue();
        }
    }
    ...
    private static Path locateDocsRulesDir() {
        ...
        for (int i = 0; i < 6 && dir != null; i++) {
            Path candidate = dir.resolve("docs").resolve("rules");
        ...
        throw new IllegalStateException(
                "Could not locate docs/rules directory from " + Paths.get("").toAbsolutePath());
    }
}
